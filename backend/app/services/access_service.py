...
async def require_project_access(
    ...
    *,
    ...
    global_scope = await has_permission(
        db,
        user,
        (
            "aquaponics_systems.manage_all"
            if manage
            else "aquaponics_systems.read_all"
        ),
    )
    scoped_read = global_scope or await has_permission(
        db,
        user,
        "aquaponics_systems.read",
        project_id,
    )
    ...
    if not global_scope:
        query = query.where(
            ...
            or_(
                ...
                Project.members.any(user_id=user.id),
            ),
        )
...
async def get_admin_project_for_lifecycle(
    db: AsyncSession,
    project_id: int,
) -> Project:
