...
import { isOperatorConsole, readAccountRole } from "@/app/lib/operator-console"
...
export function ProfilePage() {
  ...
  const cardClass = operator ? "rounded-2xl border-0 shadow-[0_8px_30px_-18px_rgba(15,63,53,0.35)]" : undefined
  return (
    <section className={cn("grid gap-5", operator ? "max-w-5xl lg:grid-cols-2" : "max-w-4xl lg:grid-cols-2")}>
      {forced ? (
        <Alert className="lg:col-span-2 border-amber-200 bg-amber-50 text-amber-900">
          <KeyRound />
          <AlertTitle>Bạn cần đổi mật khẩu</AlertTitle>
          <AlertDescription>Hoàn tất đổi mật khẩu trước khi xem Tổng quan, Thiết bị, Cảnh báo và Cài đặt của hệ thống.</AlertDescription>
        </Alert>
      ) : null}
      <Card className={cardClass}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><UserRound className="size-5 text-emerald-700" />Hồ sơ người dùng</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className={cardClass}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><KeyRound className="size-5 text-emerald-700" />Đổi mật khẩu</CardTitle>
        </CardHeader>
        <CardContent>
          <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); password.mutate() }}>
            <TextField id="current-password" label="Mật khẩu hiện tại" value={currentPassword} type="password" minLength={8} onChange={setCurrentPassword} />
            <TextField id="new-password" label="Mật khẩu mới" value={newPassword} type="password" minLength={8} onChange={setNewPassword} />
            <TextField id="confirm-password" label="Xác nhận mật khẩu mới" value={confirmPassword} type="password" minLength={8} onChange={setConfirmPassword} />
            {passwordMismatch ? <p role="alert" className="text-sm text-destructive">Mật khẩu xác nhận không khớp.</p> : null}
            <Button type="submit" className={operator ? "bg-emerald-700 hover:bg-emerald-800" : undefined} disabled={password.isPending || passwordMismatch || newPassword.length < 8}>
              <KeyRound />Đổi mật khẩu
            </Button>
            {password.isError ? <p role="alert" className="text-sm text-destructive">{errorMessage(password.error)}</p> : null}
            <p className="text-xs text-muted-foreground">Sau khi đổi mật khẩu, bạn đăng nhập lại để vào màn hình Tổng quan của hệ thống.</p>
          </form>
        </CardContent>
      </Card>
    </section>
